...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("smart_trader.log"), logging.StreamHandler()],
)
...
class SmartTraderPro:
    ...
    async def execute_trade(self, symbol, action):
        ...
        if action == "buy":
            order = await self.client.create_order(
                symbol=symbol, side="BUY", type="MARKET", quantity=round(amount, 4)
            )
            ...
            order = await self.client.create_order(
                symbol=symbol, side="SELL", type="MARKET", quantity=round(amount, 4)
            )
    ...
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text(
            "🚀 Smart Trading Bot", reply_markup=InlineKeyboardMarkup(keyboard)
        )
